package c8y.to.aris.tbl.creation.ms.service;
...
import c8y.IsDevice;
import c8y.to.aris.tbl.creation.ms.App;
import c8y.to.aris.tbl.creation.ms.connector.ArisConnector;
import c8y.to.aris.tbl.creation.ms.connector.ArisResponse;
import c8y.to.aris.tbl.creation.ms.integration.ArisDatasetManager;
...
public class ArisIntegrationService<I extends ArisConnector> {
	...
	private void updateAgent(Date toDate)
	{
		ManagedObjectRepresentation mor = new ManagedObjectRepresentation();
		mor.setId(this.microserviceAgent.getId());
		SimpleDateFormat sdf;
		sdf = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss.SSS");
		mor.setProperty("latestFetchedDate", sdf.format(toDate));
	...
	private List<List<Object>> retrieveLatestMeasurements()
	{
		...
		toDate = new Date();
		updateAgent(toDate);
		MeasurementCollection measurements = measurementApi.getMeasurementsByFilter(new MeasurementFilter().byDate(fromDate, toDate));
	...
	private void manageCycleStatesDuringDataUpload(String ingestionCycleKey, CycleState cycle)
	{
		if (cycle.getValue().compareTo("COMPLETED_SUCCESSFULLY") == 0) {
			...
			try {
				Thread.sleep(10000);
				retrieveCycleState(ingestionCycleKey);
			} catch (InterruptedException e) {
				e.printStackTrace();
			} 
	...
	private void cancelCycle(String ingestionCycleKey)
	{
		...
		if (cycle.isOk()) {
			logger.info("The cycle state has been cancelled...");
			logger.info("Trying the whole data upload steps again in 60 seconds...");
			generateEventForMicroservice("The ARIS cycle state has been cancelled... Waiting for 60 seconds before trying the data upload again.", "ms_arisApiResponse");
